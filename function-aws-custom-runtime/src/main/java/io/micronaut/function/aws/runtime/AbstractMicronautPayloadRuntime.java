...
import io.micronaut.function.aws.proxy.MicronautRequestHandlerCreator;
...
public class AbstractMicronautPayloadRuntime<RequestType, ResponseType> implements ApplicationContextProvider,
    AwsLambdaRuntimeApi {
  ...
  protected MicronautRequestHandlerCreator<RequestType, ResponseType> creator;
   ...
   * Constructor.
   */
  public AbstractMicronautPayloadRuntime() {
  }

  /**
   * Constructor.
   */
  public AbstractMicronautPayloadRuntime(MicronautRequestHandlerCreator<RequestType, ResponseType> handlerCreator) {
   ...
   */
  @Nullable
  protected RequestHandler<RequestType, ResponseType> createRequestHandler(String... args) {
    return null;
  }
  ...
  protected Object createHandler(String... args) {
    RequestHandler<RequestType, ResponseType> requestHandler = creator.createRequestHandler();
  ...
  protected void startRuntimeApiEventLoop(@NonNull URL runtimeApiURL,
    ...
    try {
      ...
      try {
          ...
          if (request != null) {
            ...
            try {
              if (StringUtils.isNotEmpty(requestId)) {
                ...
                ResponseType handlerResponse = null;
                if (handler instanceof RequestHandler) {
                  handlerResponse = ((RequestHandler<RequestType, ResponseType>) handler).handleRequest(request, context);
                }
        ...
        if (Objects.nonNull(endpointClient)) {
  ...
  protected HttpRequest decorateWithUserAgent(HttpRequest<?> request) {
    if (Objects.nonNull(userAgent) && request instanceof MutableHttpRequest) {
