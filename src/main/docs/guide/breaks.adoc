...
** The legacy bootstrap/context distributed configuration clients for AWS Parameter Store and AWS Secrets Manager are deprecated. Prefer `micronaut.config.import=parameterstore:/...` and `micronaut.config.import=aws-secretsmanager:/...` in `application.yml` instead of enabling the bootstrap config client path.
