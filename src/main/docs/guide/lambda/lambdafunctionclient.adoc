...
To configure credentials for invoking the function you can either define a `~/.aws/credentials` file or use `application.yml`. Micronaut registers a api:aws.sdk.v1.EnvironmentAWSCredentialsProvider[] that resolves AWS credentials from the Micronaut Environment.
