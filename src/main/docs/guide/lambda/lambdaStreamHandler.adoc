...
[.lang-python]
NOTE: A Python class cannot extend the Java class `MicronautRequestStreamHandler`, so the handler configured in AWS Lambda is a Java, Kotlin or Groovy class. The `@FunctionBean` it resolves, such as the `EventLogger` above, can be written in Python: keep the default handler and set the `micronaut.function.name` property, or use the Java `Handler` shown here.