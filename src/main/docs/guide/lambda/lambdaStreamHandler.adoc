...
snippet::io.micronaut.docs.function.aws.Handler[tags="clazz", languages="java,kotlin,groovy"]

[.lang-python]
NOTE: The AWS Lambda runtime instantiates the handler class before any Micronaut application context (and with it the Python runtime) exists, so the handler configured in AWS Lambda is a Java, Kotlin or Groovy class. The `@FunctionBean` it resolves, such as the `EventLogger` above, can be written in Python: keep the default handler and set the `micronaut.function.name` property, or use the Java `Handler` shown here.